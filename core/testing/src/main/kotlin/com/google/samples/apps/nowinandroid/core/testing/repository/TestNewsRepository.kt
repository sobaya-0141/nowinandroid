...
class TestNewsRepository : NewsRepository {
     ...
     */
    private val newsResourcesFlow: MutableSharedFlow<List<sobaya.app.sharemodel.NewsResource>> =
        MutableSharedFlow(replay = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)

    override fun getNewsResources(query: NewsResourceQuery): Flow<List<sobaya.app.sharemodel.NewsResource>> =
        newsResourcesFlow.map { newsResources ->
            ...
            query.filterTopicIds?.let { filterTopicIds ->
                result = newsResources.filter {
                    it.topics.map(sobaya.app.sharemodel.Topic::id).intersect(filterTopicIds).isNotEmpty()
                }
            }
     ...
     */
    fun sendNewsResources(newsResources: List<sobaya.app.sharemodel.NewsResource>) {
